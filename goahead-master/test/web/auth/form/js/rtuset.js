...
function MyGetJSON(msg,url,node,data,callback)
{
	...
	success: function(data){
		...
		if(data.ret != null ) {
			if( data.ret == 401 ) {
				window.location.href="http://"+window.location.hostname+"/auth/form/login.html";
...
function onExtTableItemClick(tb,row)
{
	if( row != null && row.rowIndex != null && row.rowIndex >= 0 ) {
		...
		setCheckBoxEnable('var_ext_dev_rtu_self', PROTO_DEV_RTU_SELF == xVarManageExtDataBase[_rowIndex]['io'].dt);
		...
		} else if(__proto_is_dlt645(xVarManageExtDataBase[_rowIndex]['io'].dt, xVarManageExtDataBase[_rowIndex]['io'].pt)) {
...
function varExtGetProto(dev,proto)
{
	...
	case PROTO_DEV_RS1: case PROTO_DEV_RS2: case PROTO_DEV_RS3: case PROTO_DEV_RS4: {
		...
		case PROTO_DLT645: return "DLT645";
...
{
	if(Number(dev) != PROTO_DEV_RTU_SELF) {
...
function checkVarAddr(_addr, _n)
{
	if( xVarManageExtDataBase == null || _n >= xVarManageExtDataBase.length ) {
		for(var n = 0; n < xVarManageExtDataBase.length; n++) {
			if( xVarManageExtDataBase[n]['io'].ad != null && xVarManageExtDataBase[n]['io'].ad == _addr ) {
				return n;
			}
		}
	} else {
		for(var n = 0; n < xVarManageExtDataBase.length; n++) {
			if( xVarManageExtDataBase[n]['io'].ad != null && n != _n && xVarManageExtDataBase[n]['io'].ad == _addr ) {
				return n;
			}
		}
	}
...
function setVarExtInfo()
{
	...
	if( id >= 0 && id <= 64 ) {
		...
		if (chk(_var_ext_name, "采集变量名称")) return;
		...
		var _io = {
			...
			dt:getChecked('var_ext_dev_rtu_self')?PROTO_DEV_RTU_SELF:Number(proto[0]), 
		...
		var __index = checkVarAddr(addr, id);
		if( __index >= 0 ) {
...
function ext_onchange_proto(_resize)
{
	...
	setDisplay('ext_dlt546_div', true);
	...
	if(protoTypeList.options.length > 0 && protoDevList.options.length > 0 ) {
		...
		if(devary.length==2) {
			...
			if(__proto_is_modbus(_dev,_po)) {
				...
				setDisplay('ext_dlt546_div', false);
				if(_resize) {
			...
			} else if(__proto_is_dlt645(_dev,_po)) {
				setDisplay('ext_dlt546_div', true);
				if(_resize) {
					setValue('var_ext_vartype0', 7);
					setValue('var_ext_vartype1', 7);
...
{
	MyGetJSONWithArg("正在设置GPRS模块,请稍后...","/cgi-bin/getGPRSCfg?", "", function (res) {
		if( res != null && 0 == res.ret ) {
			refreshGPRSInfo(res);
		} else {
			alert("读取失败,请重试");
...
function setGPRSInfo()
{
	...
	MyGetJSONWithArg("正在设置GPRS模块,请稍后...","/cgi-bin/setGPRSCfg?", JSON.stringify(setval), function (res) {
		if( res != null && 0 == res.ret ) {
			alert( "设置成功" );
		} else {
			alert("设置失败,请重试");
...
function Page_System_Resource(info)
{
	...
	//内存剩余
	document.getElementById("sys_memory_free").innerHTML = "Free : " + (Number(info.ms)-Number(info.mu)) + " B  [Total : " + Number(info.ms) + " B, Used : " +  Number(info.mu) + " B, Max Used : " +  Number(info.ma) + " B]";
...
function Apply_Remote_Transmission_downlist()
{
	...
	//以太网工作模式 
	var remote_socket_type = document.getElementById("remote_socket_type").value;
	...
	var remote_modbus_addr = document.getElementById("remote_modbus_addr").value;
	
	var remote_wm_xfer_proto_type = document.getElementById("remote_wm_xfer_proto_type").value;
	var remote_xfer_gw_trt_dst_type = document.getElementById("remote_xfer_gw_trt_dst_type").value;
	var remote_xfer_gw_trt_dst_uart_type = document.getElementById("remote_xfer_gw_trt_dst_uart_type").value;
	var remote_xfer_gw_trt_dst_uart_baudrate = document.getElementById("remote_xfer_gw_trt_dst_uart_baudrate").value;
	var remote_xfer_gw_trt_dst_uart_bits = document.getElementById("remote_xfer_gw_trt_dst_uart_bits").value;
...
function show_GPRS_btn(){
	...
	case PROTO_CC_BJDC: case PROTO_HJT212:
...
function setGPRSFileToTextarea()
{
	if(getValue('GPRS_proto')==PROTO_HJT212) {
		$.ajax({
			type: "get",
			url: "/download/cfg/hjt212_"+(4+getNumber('GPRS_group'))+".ini",
			dataType: "html",
			data: "",
			cache: false,
			success: function(data){
...
function saveGPRSConfig()
{
	...
	if (xhr) {
		...
		xhr.open("POST", "/ini/upload/cfg/hjt212_"+(4+getNumber('GPRS_group'))+".ini" );
		xhr.send(_val);
	}
}
...
function Apply_gprs_Transmission_downlist()
{
	...
	var GPRS_socket_type = document.getElementById("GPRS_socket_type").value;
	...
	var GPRS_modbus_addr = document.getElementById("GPRS_modbus_addr").value;
	
	var GPRS_wm_xfer_proto_type = document.getElementById("GPRS_wm_xfer_proto_type").value;
	var GPRS_xfer_gw_trt_dst_type = document.getElementById("GPRS_xfer_gw_trt_dst_type").value;
	var GPRS_xfer_gw_trt_dst_uart_type = document.getElementById("GPRS_xfer_gw_trt_dst_uart_type").value;
	var GPRS_xfer_gw_trt_dst_uart_baudrate = document.getElementById("GPRS_xfer_gw_trt_dst_uart_baudrate").value;
	var GPRS_xfer_gw_trt_dst_uart_bits = document.getElementById("GPRS_xfer_gw_trt_dst_uart_bits").value;
	...
	MyGetJSONWithArg("正在设置GPRS TCP/IP配置","/cgi-bin/setTcpipCfg?",JSON.stringify(setval));
}
...
{
	//*注：在此处编写获取代码；然而调用如下函数，实现页面显示；
	MyGetJSONWithArg("正在获取GPRS工作参数","/cgi-bin/getGPRSWorkCfg?","", function (cfg)
	{
...
function Apply_GPRS_Config()
{
	...
	MyGetJSONWithArg("正在设置GPRS工作参数","/cgi-bin/setGPRSWorkCfg?",JSON.stringify(setval));
}
...
function Show_GPRS_Wireless()
{
	...
	MyGetJSONWithArg("正在获取GPRS配置","/cgi-bin/getGPRSNetCfg?","", function (info)
	{
...
function Apply_GPRS_Wireless()
{
	...
	MyGetJSONWithArg("正在设置GPRS配置信息","/cgi-bin/setGPRSNetCfg?",JSON.stringify(setval));
}
...
function onSerialProtoChange()
{
	...
	setDisplay('Serial_Port_proto_nlua_ul', getNumber('Serial_Port_proto') != PROTO_LUA && getNumber('Serial_Port_proto') != PROTO_DLT645 );
	if(getNumber('Serial_Port_proto') == PROTO_LUA) {
		tryResreshLuaSelectList('Serial_Port_proto_lua_list');
	}
	setDisplay('Serial_Port_proto_ms_addr', (getNumber('Serial_Port_proto_ms') == 0)&&(getNumber('Serial_Port_proto')!=PROTO_LUA)&&(getNumber('Serial_Port_proto')!=PROTO_DLT645) );
}
...
{	
	if( row != null && row.rowIndex != null && row.rowIndex >= 0 ) {
		...
		$.ajax({
			type: "get",
			url: "/download/lua/"+getLuaName(luascript_title)+".lua",
			dataType: "html",
			data: "",
			cache: false,
			success: function(data){
...
function Save_LuaScript()
{
	...
	if (confirm(msg)){
		...
		if (xhr) {
			...
			xhr.open("POST", "/lua/upload/lua/"+luascript_title+".lua" );
...
function resetdev()
{
	if( window.confirm('确定要重启设备吗?\n\n重启大约需要15秒钟') ){
		MyGetJSONWithArg("","/action/devReset?","",function( cfg ){
...
function downloadCfg()
{
	MyGetJSONWithArg("","/cgi-bin/saveCfgWithJson?","",function(res){
		if( res != null && 0 == res.ret ) {
			window.open("http://"+window.location.hostname+"/download/cfg/board_json.cfg");
		} else {
			alert("获取配置失败，请检查后重试！");
		}
	});
...
function show_remote_btn(){
	...
	case PROTO_CC_BJDC: case PROTO_HJT212:
		...
		setEnable('remote_socket_type', false);
...
function setFileToTextarea()
{
	if(getNumber('remote_proto')==PROTO_CC_BJDC) {
		...
		$.ajax({
			type: "get",
			url: "/download/cfg/hjt212_"+getValue('remote_group')+".ini",
			dataType: "html",
			data: "",
			cache: false,
			success: function(data){
...
function saveRemoteConfig()
{
	...
	if (xhr) {
		...
		if(getNumber('remote_proto')==PROTO_CC_BJDC) {
			xhr.open("POST", "/ini/upload/cfg/cc_bjdc_"+getValue('remote_group')+".ini" );
		} else if(getNumber('remote_proto')==PROTO_HJT212) {
			xhr.open("POST", "/ini/upload/cfg/hjt212_"+getValue('remote_group')+".ini" );
		}
		xhr.send(_val);
	}
}
